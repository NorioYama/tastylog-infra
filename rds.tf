...
# #-----------------------------
# # RDS subnet group 複数のサブネットをグループとして扱う
# #-----------------------------
...
resource "aws_db_instance" "mysql_standalone" {
  ...
  publicly_accessible    = false #パブリックアクセスの許可有無
  port                   = 3306
  ...
  backup_window              = "04:00-05:00" #バックアップの時間
  backup_retention_period    = 7             #保管期間日数
  maintenance_window         = "Mon:05:00-Mon:08:00"
  auto_minor_version_upgrade = false #自動的なマイナーバージョンアップ

  deletion_protection = false #自動削させない設定にするか
  skip_final_snapshot = true  #削除時のスナップショットをスキップするか
